<?php
/**
 * Admin-only second-batch building profile importer.
 *
...
final class Calgary_Condo_Building_Second_Batch_Importer {
    private const PAGE_SLUG = 'ccl-building-second-batch-importer';
    private const DRY_RUN_ACTION = 'ccl_building_second_batch_importer_dry_run';
    private const IMPORT_ACTION = 'ccl_building_second_batch_importer_import';
    private const DRY_RUN_NONCE_ACTION = 'ccl_building_second_batch_importer_dry_run_nonce';
    private const IMPORT_NONCE_ACTION = 'ccl_building_second_batch_importer_import_nonce';
    private const BATCH_DATA_FILE = CCL_PLUGIN_DIR . 'data/building-second-batch.php';
    ...
    public function register_admin_page(): void {
        add_management_page(
            __('Calgary Building Second Batch Import', 'calgary-condo-leads'),
            __('Calgary Building Second Batch Import', 'calgary-condo-leads'),
            'manage_options',
            self::PAGE_SLUG,
    ...
    public function render_admin_page(): void {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Calgary Building Second Batch Import', 'calgary-condo-leads'); ?></h1>
            <p><?php esc_html_e('Imports the second production building batch from a structured PHP source array. The importer is repeatable and updates existing matching posts instead of creating duplicates.', 'calgary-condo-leads'); ?></p>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <?php wp_nonce_field(self::DRY_RUN_NONCE_ACTION, 'ccl_building_second_batch_importer_nonce'); ?>
                <?php submit_button(__('Run dry-run second batch import', 'calgary-condo-leads')); ?>
            </form>
            ...
            <?php if (is_array($dry_run_data) && !empty($dry_run_data['summary'])) : ?>
                ...
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    ...
                    <?php wp_nonce_field(self::IMPORT_NONCE_ACTION, 'ccl_building_second_batch_importer_nonce'); ?>
                    <label>
                        <input type="checkbox" name="confirm_import" value="yes" required />
                        <?php esc_html_e('I confirm I want to create/update ccl_building posts from the second batch source.', 'calgary-condo-leads'); ?>
                    </label>
    ...
    public function handle_dry_run(): void {
        $this->assert_admin_request(self::DRY_RUN_NONCE_ACTION, 'ccl_building_second_batch_importer_nonce');
    ...
    public function handle_import(): void {
        $this->assert_admin_request(self::IMPORT_NONCE_ACTION, 'ccl_building_second_batch_importer_nonce');
    ...
    private function dry_run_transient_key(): string {
        return 'ccl_building_second_batch_importer_dry_run_' . get_current_user_id();
    }

    private function import_transient_key(): string {
        return 'ccl_building_second_batch_importer_import_' . get_current_user_id();
    }
...
new Calgary_Condo_Building_Second_Batch_Importer();
